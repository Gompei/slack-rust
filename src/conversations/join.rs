...
mod test {
  ...
  "channel": "C1234567890"
        ...
        let s = serde_json::from_str::<JoinRequest>(json).unwrap();
        assert_eq!(request, s);
    }
  ...
  "channel": {
    ...
    "priority": 0
        ...
        let s = serde_json::from_str::<JoinResponse>(json).unwrap();
  ...
  "channel": {
    ...
    "priority": 0
        ...
        let response = join(&mock, &param, &"test_token".to_string())
            .await
            .unwrap();
        let expect = JoinResponse {
            ok: true,
            channel: Some(Channel {
