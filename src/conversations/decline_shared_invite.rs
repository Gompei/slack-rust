...
mod test {
    ...
    #[test]
    fn convert_request() {
        let request = DeclineSharedInviteRequest {
  ...
  "target_team": "xxxxxxxxxxx"
        ...
        let s = serde_json::from_str::<DeclineSharedInviteRequest>(json).unwrap();
        assert_eq!(request, s);
    }

    #[async_std::test]
    async fn test_decline_shared_invite() {
        let param = DeclineSharedInviteRequest {
            invite_id: "xxxxxxxxxxx".to_string(),
            target_team: Some("xxxxxxxxxxx".to_string()),
        };
  ...
  "ok": true
        ...
        let response = decline_shared_invite(&mock, &param, &"test_token".to_string())
            .await
            .unwrap();
